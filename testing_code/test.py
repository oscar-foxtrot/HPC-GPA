...
'''
for i in range(len(X_list)):
    print(res_gpa[0], res_gpa[1], res_gpa[2])
'''

for i in range(len(res_gpa[1])):
    matrix = np.loadtxt(f"aligned_X_{i}.txt")
    print(np.trace((matrix - res_gpa[1][i]) @ (matrix - res_gpa[1][i]).T))

matrix = np.loadtxt("Y_consensus.txt")
print(np.trace((res_gpa[0] - matrix) @ (res_gpa[0] - matrix).T))